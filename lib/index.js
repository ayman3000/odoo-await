...
* Modified by ayman hamed to enable sending db, username and password in each request
*/
...
class OdooAwait {
   ...
   * @param {string} db - e.g. 'db'
   * @param {string} uid - e.g. '0'
   ...
   */
  execute_kw(model, method, params, db, uid, password) {
    ...
    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      params.unshift(db, uid, password, model, method);
    } else {
   ...
   */
  async create(model, params = {}, externalId, moduleName, db, uid, password) {

    params = this.parseManyFields(params);
    var recordId;
    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      recordId = await this.execute_kw(model, 'create', [params], db, uid, password);
    } else {

      recordId = await this.execute_kw(model, 'create', [[params]]);
    }

    // create external ID if it was supplied
    if (externalId) {
      if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
        await this.createExternalId(model, recordId, externalId, moduleName || '__api__', db, uid, password);
      } else {
   ...
   */
  async read(model, recordId, fields = [], db, uid, password) {
    ...
    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      return await this.execute_kw(model, 'read', params, db, uid, password);
    } else {
   ...
   */
  async update(model, recordId, params = {}, db, uid, password) {

    params = this.parseManyFields(params);

    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      return await this.execute_kw(model, 'write', [[recordId], params], db, uid, password);
    } else {
   ...
   */
  async delete(model, recordId, db, uid, password) {
    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      await this.execute_kw(model, 'unlink', [[recordId]], db, uid, password);
    } else {
  ...
  async searchRead(model, domain, fields = [], opts = { offset: 0, limit: 0, order: '', context: {}, },
    ...
    var result;
    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      result = await this.execute_kw(model, 'search_read', params, db, uid, password);
    } else {

      result = await this.execute_kw(model, 'search_read', params);
    }
   ...
   * @return {Promise<Array>}
   */
  async search(model, domain, db, uid, password) {
    ...
    params.push([domainArray]);
    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      return await this.execute_kw(model, 'search', params, db, uid, password);
    } else {
   ...
   * @return {Promise<object>}
   */
  async getFields(model, attributes = [], db, uid, password) {

    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      return await this.execute_kw(model, 'fields_get', [[], { 'attributes': attributes }], db, uid, password);
    } else {
   ...
   */
  async action(model, action, recordId, db, uid, password) {

    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      return await this.execute_kw(model, action, [[recordId]], db, uid, password);
    } else {
   ...
   * @returns {Promise<*>}
   */
  async createExternalId(model, recordId, externalId, moduleName, db, uid, password) {


    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      const result = await this.execute_kw('ir.model.data', 'create', [[
        {
          model: model,
          name: `${externalId}`,
          res_id: recordId,
   ...
   * @returns {Promise<*>}
   */
  async searchByExternalId(externalId, db, uid, password) {
    var irModelData;

    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      irModelData = await this.execute_kw('ir.model.data', 'search', [[['name', '=', `${externalId}`]]], db, uid, password);
    } else {
      irModelData = await this.searchRead('ir.model.data', { name: externalId }, ['res_id']);
    }
  ...
  async updateByExternalId(externalId, params = {},
    ...
    var irModelData;
    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      irModelData = await this.searchRead('ir.model.data', { name: externalId }, ['res_id', 'model'], db, uid, password);
    } else {
      irModelData = await this.searchRead('ir.model.data', { name: externalId }, ['res_id', 'model']);
    }
    if (!irModelData.length) {
      throw new Error(`No matching record found for external identifier ${externalId}`);
    }
    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      return await this.update(irModelData[0].model, irModelData[0].res_id, params, db, uid, password);
    } else {
   ...
   */
  async deleteByExternalId(externalId, db, uid, password) {
    var irModelData;
    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      irModelData = await this.searchRead('ir.model.data', { name: externalId }, ['res_id', 'model'], db, uid, password);
    } else {
    ...
    if (typeof db != 'undefined' && typeof uid != 'undefined' && typeof password != 'undefined') {
      return await this.delete(irModelData[0].model, irModelData[0].res_id, db, uid, password);
    } else {
